...
sleep 5
echo "CREATE ROLE openerp WITH SUPERUSER CREATEDB LOGIN PASSWORD 'openerp';create database openerp with owner=openerp;SELECT rolname FROM pg_roles;\q"|/bin/su postgres -c "psql -U postgres"
